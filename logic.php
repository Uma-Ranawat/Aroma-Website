...
//            echo "<a href=home.php>Click here</a> to Login";
 ...
 elseif ($_GET['value'] == 'everify') {
    $email = $_SESSION['Email'];
    $codee = rand(0, 999999999);
    $get_Mailer_Name_q = "SELECT FirstName,LastName FROM user WHERE Email='$email'";
    $Mailer_Data = mysqli_query($con, $get_Mailer_Name_q);
    while ($mailRow = mysqli_fetch_array($Mailer_Data)) {
        $Mail_fname = $mailRow['FirstName'];
    ...
    //Set the subject line
    $mail->Subject = 'Aroma Creations Verify Email';
...
//        $mail->msgHTML(file_get_contents('contents.html'), dirname(__FILE__));
    ...
    $txt = "<body>"
            ...
            . "<table style='border:none'>"
            . "<tr>"
            . "<td><td><center><b style='font-size: 130%;color:#1a237e'>Aroma Creations</b></center></td>"
            . "</tr>"
            . "<tr>"
            . "<td><hr/></td>"
            . "</tr>"
            ...
            . "<center><h3 style='background-color: whitesmoke'>"
            . "<font color='black'>From AromaCreations <br/></font>"
            . "</h3>If you haven't done this,then please contact the administrator and reset your password.</center>"
            . "</td>"
            . "</tr>"
            . "</table>"
            . "</center></body>";
...
elseif ($_GET['value'] == 'verified') {
    ...
    if ($code!="" || $code!=null) {
        ...
        if ($row['code'] == $code) {
            ...
            $sql1 = "update user set verify='y' where Email='" .$_SESSION['Email'] . "'";
            if (mysqli_query($con, $sql1)) {
                echo "Done";
            } else {
                echo "error";
